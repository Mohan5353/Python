...
if __name__=="__main__" :
    ...
    net.eval()
    x = io.imread('./sample_inp.jpeg')
    x = transforms.ToTensor()(x).unsqueeze(dim=0)
    y = net(x)
    y = F.softmax(y,dim=1)
    idx = torch.argmax(y,dim=1)
    print('\n')
    if idx == 0:
      print('Prediction : CNV')
    elif idx == 1:
      print('Prediction : DME')
    elif idx == 2:
      print('Prediction : DRU')
    else:
      print('Prediction : NRM')
    print('\n')
   
    print('Score CNV : ', y[0,0].detach().data.numpy().round())
    print('Score DME : ', y[0,1].detach().data.numpy().round())
    print('Score DRU : ', y[0,2].detach().data.numpy().round())
    print('Score NRM : ', y[0,3].detach().data.numpy().round())
